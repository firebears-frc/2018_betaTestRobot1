...
public class RobotReport {
    ...
    private Map<PcmAddress, Tuple> pcmMap = new TreeMap<>();
    ...
    public void addPcm(int moduleNumber, int forwardChannel, int reverseChannel, String desc, Object component) {
	PcmAddress pcmAddress = new PcmAddress(moduleNumber, forwardChannel, reverseChannel);
	pcmMap.put(pcmAddress, new Tuple(desc, component));
    }

    public void addPcm(int forwardChannel, int reverseChannel, String desc, Object component) {
	PcmAddress pcmAddress = new PcmAddress(forwardChannel, reverseChannel);
	pcmMap.put(pcmAddress, new Tuple(desc, component));
    }
	...
	if (!pcmMap.isEmpty()) {
	    out.println("## Pneumatics");
	    out.println();
	    for (PcmAddress pcmAddress : pcmMap.keySet()) {
		out.printf("* %s = %s%n", pcmAddress.toString(), pcmMap.get(pcmAddress));
	    }
	    out.println();
	}
    ...
    private class PcmAddress implements Comparable<PcmAddress> {

	final int moduleNumber;
	final int forwardChannel;
	final int reverseChannel;

	public PcmAddress(int moduleNumber, int forwardChannel, int reverseChannel) {
	    this.moduleNumber = moduleNumber;
	    this.forwardChannel = forwardChannel;
	    this.reverseChannel = reverseChannel;
	}

	public PcmAddress(int forward, int reverse) {
	    moduleNumber = -1;
	...
	public String toString() {
	    if (moduleNumber < 0) {
		return "[" + forwardChannel + "," + reverseChannel + "]";
	    } else {
		return "[" + moduleNumber + "," + forwardChannel + "," + reverseChannel + "]";
	    }
	}

	@Override
	public int compareTo(PcmAddress o) {
	    if (this.moduleNumber != o.moduleNumber) {
		return this.moduleNumber - o.moduleNumber;
	    } else if (this.forwardChannel != o.forwardChannel) {
		return this.forwardChannel - o.forwardChannel;
	    } else {
		return this.reverseChannel - o.reverseChannel;
	    }
	}
    }
}
